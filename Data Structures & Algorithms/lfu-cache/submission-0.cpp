class LFUCache {
    struct ListNode{
        ...
        int freq;
        ListNode* next,*prev;
        ListNode(int key, int val) : key(key), val(val), freq(1), prev(nullptr),
        next(nullptr){}
    };
    ...
    int capacity;
    int lfuCount;
    unordered_map<int,ListNode*> nodeMap; // key -> Node mapping (to get the node)
    unordered_map<int,LinkedList*> listMap; // count -> doubly linked list mapping
    // to get the list from which we need to remove (least frequent one)
